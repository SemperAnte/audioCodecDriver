                       ...
                       alt_u8  infoLevel )    // info level ( 0, 1 - additional debug info )
{
   ...
   for ( attNum = 0; attNum < 3; attNum++ ) { // set max number of attempts
      ...
      for ( cmdNum = 0; cmdNum < sizeof( cmdData ) / sizeof( cmdData[ 0 ] ); cmdNum++ ) {
         ...
         err = ssm2603I2cWrite( baseAdr, devAdr, cmdData[ cmdNum ].regAdr, cmdData[ cmdNum ].regData );
         if ( !err ) {
            if ( infoLevel )
               printf( "ok\n" );
         } else {
         ...
         err = ssm2603I2cRead( baseAdr, devAdr, cmdData[ cmdNum ].regAdr, &readData );
         if ( !err ) {
            if ( infoLevel )
               printf( "data 0x%03X\n", readData );
         } else {
                       ...
                       alt_u16* regData )   // read data here, 9 bits
{
   ...
   IOWR_8DIRECT( baseAdr, 2, ( devAdr << 1 ) | 0x01 );
